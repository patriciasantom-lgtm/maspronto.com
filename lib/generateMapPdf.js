...
export async function generateMapPdfFromConfig({ theme, character, title, subtitle, font }) {
  ...
  let titleFont
  try {
    const fontId = font || 'Bubblegum Sans'
    const titleFontBytes = getTitleFontBuffer(fontId)
    // pdf-lib's font subsetter drops glyphs (confirmed: r, u, a vanish from "Bruna")
    // when subsetting Caveat-Bold.ttf — it's a variable font and pdf-lib/fontkit's
    // subsetter mishandles its glyph table. Subsetting is only a file-size optimization,
    // so skip it for Caveat and embed the full font instead; the other 5 static fonts
    // subset correctly (verified) and keep the smaller file size.
    const subset = fontId !== 'Caveat'
    titleFont = await pdfDoc.embedFont(titleFontBytes, { subset })
  } catch (err) {
