...
export async function generateMapPngFromConfig({ theme, character, title, subtitle }) {
  ...
  if (title) {
    svgParts.push(
      `<text x="${PAGE_W / 2}" y="${titleY}" text-anchor="middle" font-family="Helvetica, Arial, sans-serif" font-weight="700" font-size="${pos.title.size}" fill="#1A1A1A">${escapeXml(title.slice(0, 80))}</text>`
    )
  }
  if (subtitle) {
    svgParts.push(
      `<text x="${PAGE_W / 2}" y="${subtitleY}" text-anchor="middle" font-family="Helvetica, Arial, sans-serif" font-size="${pos.subtitle.size}" fill="#1A1A1A">${escapeXml(subtitle.slice(0, 120))}</text>`
    )
  }
  svgParts.push('</svg>')
