export const REGIONS = {
  AU: {
    ...
    currency: 'aud',
    digitalPrice: 990,
    kitProductPrice: 4600,
    kitShippingPrice: 1390,
    kitTotalPrice: 5990,
    allowedCountries: ['AU'],
  },
  US: {
...
/** Read the geo-detected region from the cookie set by middleware. Client-side only. */
export function getGeoRegion() {
  if (typeof document === 'undefined') return null
  const match = document.cookie.match(/(?:^|;\s*)pronto-geo-region=([^;]+)/)
  if (!match) return null
  const region = match[1]
  return REGIONS[region] ? region : null
}
